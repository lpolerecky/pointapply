...
diag_64_MON <-  dplyr::distinct(
  ...
  dplyr::transmute(
    ...
    p_inter = sig_coder(p_inter, make_lab = FALSE),
...
```{r MEXtab}
# table of stats MEX
diag_64_MEX <- dplyr::distinct(diag_64_MEX, dim_name.nm, M_R.grid.nm, RS_R_inter, p_inter) %>% 
  dplyr::left_join(dplyr::filter(stat_R_ext_64_all, sample.nm == "MEX"), by = "dim_name.nm") %>% 
  dplyr::transmute(
    dim_name.nm = unique(dim_name.nm),
    M_R.grid.nm = sprintf("%.4f",  M_R.grid.nm),
    RS_R_inter = sprintf("%.1f", RS_R_inter),
