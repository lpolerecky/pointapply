...
Existing data processing techniques implementing this task involve a univariate approach based on the evaluation of $R$ values obtained on the scale of measurements and analyses (Cameca^TM^ Software; [@Ramsey2018]). More specifically, they classify a specific $R$ value as anomalous if it falls outside of an interval expected for a homogeneous material based on random factors alone. However, because the variability in $R$ values detected during a particular SIMS analysis is inherently high, identifying anomalous $R$ values can be difficult (Fig. \@ref(fig:simulations)). For example, if the volume sampled during the SIMS analysis contained an inclusion, its presence could go undetected even though its $R$ value was substantially different from that of the bulk substrate. Similarly, depth-gradients of $R$ values within the sampled volume, although highly significant, could also go unnoticed. Thus, the statistical approach based on the univariate metrics ($R$) can yield erroneous conclusions about the homogeneity of the studied material.
...
In this study, we propose a new approach for detecting the underlying structure of isotopic variability in the SIMS-generated ion count data. In contrast to the existing univariate approach, which is based on the isotope ratio alone, our new approach is based on a bivariate regression of the count rates for both ions. We first describe the basic theory behind the approach and then validate it using synthetic data. Subsequently, we illustrate a real-world application of the new approach using SIMS data acquired from a carbonate mineral and a carbonate-rich sedimentary rock to. We show that compared to the existing approaches, our new method is more specific in detecting anomalous R values and allows more accurate statistical inferences about the isotopic variability within individual sputter pits (internal variability) and among multiple sputter pits on the same substrate or across multiple substrates (external variability). The visual representation of the bivariate regression additionally allows identification of the possible sources of such variability (e.g., inclusion vs. a depth-gradient).
