...
Secondary ion mass spectrometry (SIMS) is one of the techniques that allow high-accuracy isotope ratio analyses of solid-phase materials. Due to its superior spatial resolution, capability of isotope ratio analyses on light elements, and the commercial availability of high-end instruments (e.g., Cameca^TM^ 1270, 1280, nanoSIMS 50L), SIMS has been increasingly applied in the field of geology and biogeochemistry [e.g., @Fike2008; @Rollion-Bard2007; @Kozdon2009a; @Wacey2011; @Kilburn2015a; @Wei2015; @Lin2016]. At a most fundamental level these applications of SIMS will help elucidate how isotopic variations are manifested at microscopic scales and link them to emergent properties of the macroscopic world. However, the quality of such inferences rests on the accuracy of a particular SIMS analysis and requires an in-depth knowledge of the material which yielded that isotope ratio.

In a cursory overview, accuracy can be affected by a random bias attributed to quantum mechanics, dictating the chance of an atom being ejected as an secondary ion ("fundamental imprecision") [@Schroeer1973; @Shimizu1982], and a range of machine-specific systematic biases related to secondary ion production, transmission and detection [@Rudat1979; @Slodzian1980; @Shimizu1982; @Eiler1997; @Hervig2006]. The fundamental imprecision in secondary ion generation is accommodated by increasing the number of observations (increasing the $n$-series of measurements), thus optimizing the precision of an analysis according to count (or Poisson) statistics [@Fitzsimons2000a; @Kilburn2015a]. Whereas, corrections can be made for some of the systematic biases, such as the distinct time-interval of electronic paralysis (or "deadtime") of ion detection devices known as electron multipliers [@Fitzsimons2000a; @Kilburn2015a]. Other systematic biases show a strong correlation with the analysed material ("matrix effect") [@Slodzian1980; @Eiler1997], and are therefore routinely corrected by normalization with a matrix-matched reference material (RM) measured along with the target material [@Fitzsimons2000a; @Kilburn2015a]. However, these RM-based corrections are currently limited by the lack of reliable and well characterized reference materials [@Jochum2008; @Ramsey2018].

One of the factors that hamper the development of RMs suitable for SIMS analyses is the difficulty to detect whether a potential RM, be it natural or synthetic, is homogeneous on the scale of a SIMS analysis. This is complicated for the SIMS technique, as opposed to conventional gas-sourced isotope ratio mass spectrometry (IRMS) on e.g., carbonate (by means of acid digestion; e.g., gasbench or Kiel Carbonate Device) is indiscriminate towards impurities in the analyte. To assess the material homogeneity, a SIMS study typically consists of a series of $m$ analyses conducted in multiple sputter pits on the material surface (e.g., along a transect), with each analysis comprising a series of $n$ measurements along the depth in the material (Fig. \@ref(fig:sketch)). In order for the aforementioned approach to be successful one needs to validate whether: 1) the individual analyses are accurate, and 2) if the material analysed with depth of the sputter pit was homogeneous or varied among the $n$-series of measurements. In light of the second statement, it is important to observe that material heterogeneity likely manifests itself at the scale of a single analysis [@Verkouteren2004; @Jochum2008]
...
Ascertaining material homogeneity, especially at the scale of individual analysis, is thus of high importance for the development of RMs, but also the applicability of SIMS in general. In the hypothetical case of producing a carbonate-based RM, the indiscriminate nature of SIMS means that not only ^13^C/^12^C ($R$) variations within the crystal lattice, or between carbonate minerals, are a cause of concern, but also matrix impurities, such as organic carbon, with generally relatively ^13^C-depleted signatures compared to the host carbonate, and therefore the composite SIMS-based ^13^C/^12^C analysis ($n$-series of measurements) could give a biased view (intra-analysis isotope variation; Fig. \@ref(fig:sketch)). Whereas, acid-digestion during the Gasbench-IRMS procedure minimises such mixing effects of carbonate-hosted organic carbon [however, see @Oehlerich2013]. The assigned (averaged) value and uncertainty of an RM is based on a $m$-series of analysis, where intra-analyses isotope variation in the set of analyses (Fig. \@ref(fig:sketch)) can skew the averaged value. 

Existing data processing techniques for detecting variability in isotopic composition based on SIMS-generated ion count data involve univariate approaches to detect anomalous R on the scale of measurements and analyses  [Cameca^TM^ Software and @Ramsey2018]. More specifically, in this approach the $R$ value obtained during a particular measurement cycle is evaluated relative to a predefined interval (usually a band of two times the standard deviation around the mean $R$), or against “replicated” analyses. It can, however, be shown that, even though mean values for $R$ are similar, they can still be compromised with anomalous R values, e.g., if the volume sampled during the SIMS analysis contained an inclusion within an otherwise homogeneous substrate (Fig. \@ref(fig:simulations)). Moreover, the generally high variation among measurements inherent to SIMS isotope ratio analysis can entirely mask this inclusion of anomalous $R$. Hence such situations might go entirely unnoticed using univariate metrics (Fig. \@ref(fig:simulations)).  
...
In this study, we propose a new approach to detect the underlying structure of isotopic variability from SIMS-generated ion count data. A bivariate approach, not only provides a more detailed representation of the underlying structure of paired ion count ratios on a scatterplot, it also reduces to a testable statement where the variation of the individual isotope species of the same element are proportionally related through the mean $R$. The latter aspect can be approximated with a bivariate regression, which can be used to assess the influence of individual anomalous measurements or analyses on an average isotopic value. Ultimately, this new approach allows us to make statistical inferences about isotopic variability within an individual measurement spot (internal variability) as well as among multiple spots on the same substrate or across multiple substrates (external variability).
