...
# load simulated sensitivity data
...
tb_R <- point::stat_R(simu_sens_IC_intra, "13C", "12C", !!!grps, .X = Xt.sm, 
                      .N = N.sm) 
# single ions stats
tb_X <- point::stat_X(simu_sens_IC_intra, !!!grps, .X = Xt.sm, .N = N.sm) %>% 
  dplyr::filter(species.nm == "12C")
# filter and cut continuous ionization efficiency
simu_stat_eval_intra <- list(simu_CD_eval_intra, tb_X, tb_R) %>% 
  purrr::reduce(left_join, by = sapply(grps, rlang::as_name)) %>% 
  dplyr::mutate(ion_trend = RS_Xt.sm - hat_RS_N.sm) %>% 
  dplyr::filter(ion_trend > 6) %>% 
  dplyr::mutate(ion_trend =
                  factor(
...
```{r FvsChi}
# plot F vs Chi2
ggplot(simu_stat_eval_intra, aes(x = chi2_R_N.sm, y = F_R_Xt.sm, color = force.nm)) +
  geom_point() +
