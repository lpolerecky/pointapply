...
This vignette shows how ion count data of a Secondary Ion Mass Spectrometry (SIMS) isotope analysis is simulated, which forms the basis to test the intra- and inter-analysis isotope test as introduced in the paper (Section 4.1 *Simulated data*). The accompanying vignette *Visualise test performance* (`vignette("performance")`) provide the code to plot these results (Figs 4 and 5).
...
The performance of the intra- and inter-analysis isotope tests, bundled in this package, are tested with the simulated (or synthetic) data.
...
# save and print
save_point("simu_intra_concept", last_plot(), width = 14, height = 7,
           unit = "cm")
```
...
```{r saveintra}
write_point(simu_sens_IC_intra)
```


# Sensitivity intra-analysis isotope variability test

The functions `diag_R()` of the `point` package are the core functions for assessing intra-analysis isotope variability (as introduced in the paper). To gauge the performance of this test, the `simu_sens_IC_intra` is analysed by subsequently applying the `diag_R()`, to first detect potential outliers and then assess their significance in terms on their effect on the "ideal" linear R model (see paper). 


## Define groups

Both functions require grouping variable in order to define what constitutes a single analysis which, in turn, consists off the $n$-series of measurements, where the outlier are detected in the measurements (and thus gauge intra-analysis isotope variability).
...
```{r CD}
# diagnostics
simu_CD_eval_intra <- diag_R(simu_sens_IC_intra, "13C", "12C", !!!grps, 
                             .X = Xt.sm, .N = N.sm)
# export
write_point(simu_CD_eval_intra)
```
...
```{r CM}
# diagnostics
tb_dia <- diag_R(simu_sens_IC_intra , "13C", "12C", !!! grps, bl.nm, .X = Xt.sm, 
                 .N = N.sm, .method = "Cameca", .output = "diagnostic")
# evaluation of performance (intra-analysis isotope test)
CM_eval <- eval_diag(tb_dia, "13C", "12C", !!!grps, .X = Xt.sm, .N = N.sm)
# extract distinct groups omitting block-wise means
simu_CM_eval_intra <- distinct(CM_eval, !!!grps, .keep_all = TRUE)
# export
write_point(simu_CM_eval_intra)
```


# Simulate inter-analysis isotope variability data

The simulated data to assess the performance of the inter-analysis isotope variability test encompasses the same rang as for the intra-analysis isotope variability test. However, for this simulation, no intra-analysis isotope variance is included (i.e., the ideal linear R model), and only the starting value is varied over a range of 12.0‰ with again the VPDB isotope scale as a reference framework.
...
Execution of the simulation (`simu_R()`) for inter-analysis isotope data follows the same procedure as previously outlined for the intra-analysis isotope data simulation. The largest difference is that the parameter `.type` is now fixed to `"ideal`, referring to the ideal linear R model (i.e. the absence of intra-analysis isotope variability).


```{r execinter}
# execute
simu_sens_IC_inter <- pmap_dfr(ls_parms,  point::simu_R, .type = "ideal", 
                               .reps = reps, .ion1 = "13C", .ion2 = "12C", 
                               .reference = "VPDB")
```
...
ls_inter <- map(
  1:10, 
  ~inter_run(vct = var_B, n = .x, file_name = "simu_sens_IC_inter", reps = reps)
)
```
...
The intra-analysis isotope variability test is also encapsulated in the sequential usage of `diag_R()` of the `point` package. 
...
## Execute inter-analysis isotope variability test 
...
# load data
# load_point("simu", paste0("sens_IC_inter_", 1:10), NULL, return_name = FALSE)
```
...
inter_wrap <- function(simu, n, grps, file_name){
  tb_dia <- diag_R(simu, "13C", "12C", !!!grps, .X = Xt.sm, .N = N.sm)
  name <- paste(file_name, n, sep = "_")
  assign(name, tb_dia)
  exec("write_point", name)
}

# execute
purrr::iwalk(
  ls_inter, 
  ~inter_wrap(.x, .y, grps = grps, file_name = "simu_eval_inter")
)
```
