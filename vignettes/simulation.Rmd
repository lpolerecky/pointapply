...
This vignette shows how ion count data of a Secondary Ion Mass Spectrometry (SIMS) isotope analysis is simulated, which forms the basis to test the intra- and inter isotope test as introduced in the paper (Section 4.1 *Simulated data*). The accompanying vignette *Visualise test performance* (`vignette("performance")`) provide the code to plot these results (Figs 4--6).
...
gg_dens(
  ...
  ggplot2::geom_errorbarh(
    ...
    aes(xmin = M_Xt.sm - S_Xt.sm, xmax = M_Xt.sm + S_Xt.sm, y = 250),
    height = 10, 
    inherit.aes = FALSE
    ) +
  ggplot2::geom_errorbarh(
    data = filter(tb_X, species.nm == "12C"), 
    aes(xmin = M_Xt.sm - hat_S_N.sm, xmax = M_Xt.sm + hat_S_N.sm, y = 265),
    height = 10, 
    inherit.aes = FALSE
    ) +
  ggrepel::geom_text_repel(
    data = filter(tb_X, species.nm == "12C"), 
    aes(x = M_Xt.sm - S_Xt.sm, y = 250, label = "true"),
    nudge_x = -10,
    size = 3
    ) +
  ggrepel::geom_text_repel(
    data = filter(tb_X, species.nm == "12C"), 
    aes(x = M_Xt.sm - hat_S_N.sm, y = 265, label = "predicted"),
    nudge_x = -10,
    size = 3
    )

# save and print
save_point("ionization_concept", last_plot(), width = 14, height = 10, 
           unit = "cm", on_build)
```
...
# save and print
save_point("simu_intra_concept", last_plot(), width = 14, height = 5.5, unit = "cm", on_build)
```
...
```{r CD}
# diagnostics
tb_dia <- point::diag_R(simu_sens_IC_intra, "13C", "12C", !!!grps, .N = N.sm, 
                        .Xt = Xt.sm)
# evaluation of performance
simu_CD_eval_intra <- point::eval_diag(tb_dia, "13C", "12C", !!!grps, .N = N.sm, 
