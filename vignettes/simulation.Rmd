...
The $\sigma_R$-rejection method requires the `.method` to be set to `"Cameca"`. In addition, to faithfully mimic the behaviour of the Cameca software the grouping structure needs to be adjusted slightly, where this software uses an intermediate (but artificial) subdivision of a single analysis; the so-called "block" (variable: `bl.nm`), which consists generally of ~50 measurements. The application of `diag_R()` produces now mean values for the single blocks, which are irrelevant for the subsequent study, as such the `tibble` [@tibble] is filtered for distinct analysis only by using `dplyr::distinct()` [@dplyr].


```{r CM}
# diagnostics
CM_eval <- point::diag_R(simu_sens_IC_intra , "13C", "12C", !!!grps, bl.nm, 
                        .method = "Cameca")
