...
The validity of inter- and intra- isotope models is verified for the nanoSIMS-generated data by an assessment of the implicit assumptions of the linear regression model (Eq. (4) of the paper, and see below); normality, homogeneity and independence of residuals (see also `vignette("IC-diagnostics", package = "point")`). 

$$\text{The ideal linear R model:} \quad \hat{X}_i^b = \hat{R}X_i^a +  \hat{e}_i \text{.}$$
...
```{r mu}
tb_mu <- point::diag_R(tb, "13C", "12C", file.nm, sample.nm,  grid_size.nm, 
                       grid.nm, .method = "QQ", .hyp = "ttest", 
                       .output = "diagnostics")
```
...
```{r CV}
tb_CV <- point::diag_R(tb, "13C", "12C", file.nm, sample.nm,  grid_size.nm, 
                       grid.nm, .method = "CV", .hyp = "bp", 
                       .output = "diagnostics")
```
...
```{r IR}
tb_IR <- point::diag_R(tb, "13C", "12C", file.nm, sample.nm,  grid_size.nm, 
                       grid.nm, .method = "IR", .hyp = "ljung", 
                       .output = "diagnostics")  
```
...
save_point("bar_QQ_class", QQ_class, width = 7, height = 7, unit = "cm", 
           on_build) # save
```


```{r echo=FALSE, eval=TRUE, fig.cap="Anderson-Darling normality test.", out.width="50%"}
 knitr::include_graphics("figures/bar_QQ_class.png")
```


```{r CVclass}
CV_class <- hyp_class(tb_CV, vc_N_13C, "Breusch-Pagan test") # barplot
save_point("bar_CV_class", CV_class, width = 7, height = 7, unit = "cm", on_build) # save
```


```{r echo=FALSE, eval=TRUE, fig.cap="Breusch-Pagan test.", out.width="50%"}
 knitr::include_graphics("figures/bar_CV_class.png")
```


```{r muclass}
mu_class <- hyp_class(tb_mu, vc_N_13C, "Student's t test") # barplot  
save_point("bar_mu_class", mu_class, width = 7, height = 7, unit = "cm", on_build) # save
```


```{r echo=FALSE, eval=TRUE, fig.cap="Student's t test.", out.width="50%"}
 knitr::include_graphics("figures/bar_mu_class.png")
```


```{r IRclass}
IR_class <- hyp_class(tb_IR, vc_N_13C, "Ljung-box test")  # barplot
save_point("bar_IR_class", IR_class, width = 7, height = 7, unit = "cm",on_build) # save
```


```{r echo=FALSE, eval=TRUE, fig.cap="Ljung-box test.", out.width="50%"}
 knitr::include_graphics("figures/bar_IR_class.png")
```
...
# add abline
QQ_dens <- QQ_dens + ggplot2::geom_abline(alpha = 0.7, linetype = 3) 
# plot and save for paper
save_point(QQ_dens, name = "point_QQ", width = 12, height = 10, unit = "cm", 
           on_build)
```
...
# add ab line
CV_dens <- CV_dens + ggplot2::geom_hline(yintercept = 0, linetype = 3)
# plot and save for paper
save_point(CV_dens, name = "point_CV", width = 12, height = 10, unit = "cm", 
           on_build)
```
